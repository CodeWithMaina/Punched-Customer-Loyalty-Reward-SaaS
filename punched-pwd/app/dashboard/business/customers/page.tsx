      ...
      .then((res) => {
        if (!cancelled) {
          if (res.success && res.data) {
            ...
            setTotalPages(
              Math.max(1, Math.ceil(res.data.totalCount / s.pageSize)),
            );
          } else {
      ...
      <main className="mx-auto max-w-[1600px] px-4 py-5 pb-16 sm:px-6 lg:px-8 lg:py-8">
        ...
        <section className="mb-6 rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)]">
          ...
          <div className="hidden flex-wrap items-center gap-2 p-3 md:flex">
              ...
              onChange={(event) =>
                patchState({
                  status: (event.target.value ||
                    undefined) as CustomerListState["status"],
                  page: 1,
                })
              }
            ...
            <div className="ml-auto border-l border-[var(--border-light)] pl-3">
              <SortPills
                state={state}
                onCycle={(key) => patchState(cycleSort(state, key))}
              />
            </div>
          </div>
            ...
            <div className="grid gap-3 border-t border-[var(--border)] p-3 md:hidden">
              ...
              <label className="block">
                ...
                <input
                  ...
                  onChange={(event) =>
                    patchState({
                      enrolledFrom: event.target.value || undefined,
                      page: 1,
                    })
                  }
              ...
              <label className="block">
                ...
                <input
                  ...
                  onChange={(event) =>
                    patchState({
                      enrolledTo: event.target.value || undefined,
                      page: 1,
                    })
                  }
        ...
        {error ? (
          ...
          <>
            ...
            <div className="hidden overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] md:block">
              {items.map((customer, index) => (
                <CustomerRow
                  ...
                  customer={customer}
                  rank={(state.page - 1) * state.pageSize + index + 1}
                  showRank={
                    state.sortBy === "stamps" && state.sortDirection === "desc"
                  }
            ...
            {/* Shared pager + page-size selector */}
            <div className="mt-4 rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)]">
              ...
              <div className="flex items-center justify-end gap-2 border-t border-[var(--border-light)] px-5 py-2.5">
                <label
                ...
                <select
                  ...
                  className="h-8 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--background)] px-2 text-xs font-semibold text-[var(--text-primary)] outline-none focus:border-brand"
                >
        ...
        <button
          ...
          title={
            state.sortBy === opt.key
              ? `Current: ${sortLabel(state)} — tap to flip`
              : undefined
          }
